...
class FoodserviceConfig:
    ...
    MIN_VOLUME_FOR_ACTION = 0
    HIGH_RECOVERY_THRESHOLD = 0.15
...
def classify_customer_activity(df: pd.DataFrame, 
    ...
    FIXED LOGIC:
    - ACTIVE_BUYER: Pounds CY > 0 AND Last Invoice Date recent (both required)
    - LAPSED_FROM_CATEGORY: NOT active in this category BUT still buying other stuff
    - LOST_CUSTOMER: Not buying this category AND not buying anything else
    """
    ...
    def classify(row):
        """
        Classify customer based on category purchase AND overall activity.
        
        ACTIVE_BUYER = Buying this category AND active overall
        LAPSED_FROM_CATEGORY = NOT buying this category BUT active overall (THE OPPORTUNITY!)
        LOST_CUSTOMER = Not buying this category AND not active overall
        """
        
        has_category_volume = row['Has_CY_Volume']  # Pounds CY > 0
        
        # Get weeks since ANY purchase (from Last Invoice Date)
        if has_last_invoice and fiscal_calendar is not None:
            weeks_since_any = row.get('Weeks_Since_Any_Purchase', 999)
            is_active_overall = (weeks_since_any <= 8)  # Bought ANYTHING in last 8 weeks
        else:
            # Fallback if no Last Invoice Date - use category purchase as proxy
            weeks_since_category = row['Weeks_Since_Last_Purchase']
            is_active_overall = (weeks_since_category <= 8)
        
        # ============================================================
        # CLASSIFICATION LOGIC
        # ============================================================
        
        # ACTIVE_BUYER: Must have BOTH category volume AND be active overall
        if has_category_volume and is_active_overall:
            return 'ACTIVE_BUYER'
        
        # LAPSED_FROM_CATEGORY: NOT buying category BUT still active overall (THE GOLD MINE!)
        elif not has_category_volume and is_active_overall:
            return 'LAPSED_FROM_CATEGORY'
        
        # LOST_CUSTOMER: Not buying category AND not active overall
        else:
    ...
    if 'Weeks_Since_Any_Purchase' in df.columns:
        ...
        print(f"      🎯 Found {len(active_but_lapsed):,} rows where customer lapsed from category but still active overall")
        
        # Show breakdown
        lapsed_with_volume = df[
            (df['Customer_Status'] == 'LAPSED_FROM_CATEGORY') & 
            (df['Has_CY_Volume'] == True)
        ]
        if len(lapsed_with_volume) > 0:
            print(f"      ⚠️  WARNING: {len(lapsed_with_volume):,} LAPSED customers have Pounds CY > 0")
            print(f"         This shouldn't happen - check classification logic!")
    else:
        print(f"      ⚠️  Last Invoice Date NOT mapped - using fallback classification")
...
def validate_classification(df: pd.DataFrame):
    """
    Run validation checks on classified data to ensure logic is correct.
    """
    
    print("\n" + "="*60)
    print("🔍 VALIDATING CUSTOMER CLASSIFICATION")
    print("="*60)
    
    # Test 1: Active buyers should have volume AND recent invoice
    active_buyers = df[df['Customer_Status'] == 'ACTIVE_BUYER']
    
    invalid_active = active_buyers[
        (active_buyers['Has_CY_Volume'] == False) | 
        (active_buyers['Weeks_Since_Any_Purchase'] > 8)
    ]
    
    if len(invalid_active) > 0:
        print(f"\n❌ TEST 1 FAILED: {len(invalid_active):,} ACTIVE_BUYER records don't meet criteria")
        print("   Active buyers should have Pounds CY > 0 AND Last Invoice Date within 8 weeks")
    else:
        print(f"\n✅ TEST 1 PASSED: All {len(active_buyers):,} ACTIVE_BUYER records are valid")
    
    # Test 2: Lapsed should NOT have volume but SHOULD be active overall
    lapsed = df[df['Customer_Status'] == 'LAPSED_FROM_CATEGORY']
    
    invalid_lapsed_volume = lapsed[lapsed['Has_CY_Volume'] == True]
    invalid_lapsed_inactive = lapsed[lapsed['Weeks_Since_Any_Purchase'] > 8]
    
    if len(invalid_lapsed_volume) > 0:
        print(f"\n❌ TEST 2A FAILED: {len(invalid_lapsed_volume):,} LAPSED records have Pounds CY > 0")
        print("   These should be ACTIVE_BUYER, not LAPSED")
    else:
        print(f"\n✅ TEST 2A PASSED: All {len(lapsed):,} LAPSED records have no category volume")
    
    if len(invalid_lapsed_inactive) > 0:
        print(f"\n❌ TEST 2B FAILED: {len(invalid_lapsed_inactive):,} LAPSED records are inactive overall")
        print("   These should be LOST_CUSTOMER, not LAPSED")
    else:
        print(f"\n✅ TEST 2B PASSED: All {len(lapsed):,} LAPSED records are active overall")
    
    # Test 3: Lost should NOT have volume AND NOT be active overall
    lost = df[df['Customer_Status'] == 'LOST_CUSTOMER']
    
    invalid_lost = lost[
        (lost['Has_CY_Volume'] == True) | 
        (lost['Weeks_Since_Any_Purchase'] <= 8)
    ]
    
    if len(invalid_lost) > 0:
        print(f"\n❌ TEST 3 FAILED: {len(invalid_lost):,} LOST_CUSTOMER records are misclassified")
    else:
        print(f"\n✅ TEST 3 PASSED: All {len(lost):,} LOST_CUSTOMER records are valid")
    
    # Summary
    print("\n" + "="*60)
    print("📊 CLASSIFICATION SUMMARY")
    print("="*60)
    print(f"ACTIVE_BUYER:        {len(active_buyers):,} ({len(active_buyers)/len(df)*100:.1f}%)")
    print(f"LAPSED_FROM_CATEGORY: {len(lapsed):,} ({len(lapsed)/len(df)*100:.1f}%) ← THE OPPORTUNITY!")
    print(f"LOST_CUSTOMER:       {len(lost):,} ({len(lost)/len(df)*100:.1f}%)")
    print("="*60)
...
def calculate_price_elasticity(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return result_df
# ADD THESE TWO FUNCTIONS to your PZoneFinal.py (after calculate_price_elasticity)

def calculate_item_level_elasticity(historical_df: pd.DataFrame,
                                    config: FoodserviceConfig,
                                    top_n: int = 20) -> pd.DataFrame:
    """
    Calculate price elasticity for TOP 20 items by volume.
    Shows which specific SUPC items are most price-sensitive.
    """
    
    print(f"\n📊 Calculating ITEM-LEVEL price elasticity (top {top_n} by volume)...")
    
    # Check if we have item column
    if 'SUPC' not in historical_df.columns and 'Item ID' not in historical_df.columns:
        print("   ⚠️ No SUPC or Item ID column found - cannot calculate item-level elasticity")
        return pd.DataFrame()
    
    item_col = 'SUPC' if 'SUPC' in historical_df.columns else 'Item ID'
    
    df = historical_df.copy()
    
    # Get top N items by total volume
    item_volumes = df.groupby(item_col)['Pounds_CY'].sum()
    top_items = item_volumes.nlargest(top_n).index
    df = df[df[item_col].isin(top_items)]
    
    print(f"   📦 Analyzing top {len(top_items)} items by volume")
    
    # Ensure columns
    df['Net_Sales_Per_LB'] = pd.to_numeric(
        df[config.data_config.net_sales_per_lb_cy_column], 
        errors='coerce'
    )
    df['Pounds_CY'] = pd.to_numeric(df[config.data_config.pounds_cy_column], errors='coerce')
    df['Fiscal_Week'] = pd.to_numeric(df[config.data_config.fiscal_week_column], errors='coerce')
    
    df = df.dropna(subset=['Net_Sales_Per_LB', 'Pounds_CY', 'Fiscal_Week', item_col])
    
    elasticity_data = []
    
    # For each top item, detect price changes
    for item in df[item_col].unique():
        item_data = df[df[item_col] == item].copy()
        item_data = item_data.sort_values('Fiscal_Week')
        
        # Get combo for context
        combo = item_data['Company_Combo_Key'].iloc[0] if 'Company_Combo_Key' in item_data.columns else 'Unknown'
        
        # Detect price changes (5%+ jump week-to-week)
        item_data['Price_Change_Pct'] = item_data['Net_Sales_Per_LB'].pct_change() * 100
        
        price_change_weeks = item_data[abs(item_data['Price_Change_Pct']) > 5]['Fiscal_Week'].values
        
        for change_week in price_change_weeks:
            # Before: 3 weeks before change
            before_data = item_data[
                item_data['Fiscal_Week'].between(change_week - 3, change_week - 1)
            ]
            
            # After: weeks 3-5 after change (stable period)
            after_data = item_data[
                item_data['Fiscal_Week'].between(change_week + 3, change_week + 5)
            ]
            ...
            avg_volume_before = before_data['Pounds_CY'].mean()
            avg_volume_after = after_data['Pounds_CY'].mean()
            total_volume = item_volumes[item]  # Use pre-calculated total
    ...
    if not result_df.empty:
        ...
        result_df = result_df.sort_values('total_volume_lbs', ascending=False)
        print(f"   ✅ Calculated item-level elasticity for {len(result_df)} price changes across top items")
        
        # Show breakdown
        sensitivity_counts = result_df.groupby('item')['sensitivity'].first().value_counts()
        for sensitivity, count in sensitivity_counts.items():
            print(f"      • {sensitivity}: {count} items")
    else:
        print(f"   ⚠️ No item-level elasticity calculated (no price changes detected)")
    
    return result_df


def calculate_customer_item_elasticity(historical_df: pd.DataFrame,
                                       config: FoodserviceConfig,
                                       top_n: int = 20) -> pd.DataFrame:
    """
    Calculate price sensitivity for top 20 CUSTOMER + ITEM pairs by volume.
    Shows which customer+item combinations are most price-sensitive.
    """
    
    print(f"\n👥 Calculating CUSTOMER+ITEM price sensitivity (top {top_n} by volume)...")
    ...
    # Get top 20 customer+item pairs by total volume
    cust_item_volumes = df.groupby(
        [config.data_config.customer_id_column, item_col]
    )['Pounds_CY'].sum().reset_index()
    
    cust_item_volumes.columns = ['customer', 'item', 'total_volume']
    cust_item_volumes = cust_item_volumes.nlargest(top_n, 'total_volume')
    
    print(f"   👥 Analyzing top {len(cust_item_volumes)} customer+item pairs by volume")
    ...
    df['Pounds_CY'] = pd.to_numeric(df[config.data_config.pounds_cy_column], errors='coerce')
    df = df.dropna(subset=['Net_Sales_Per_LB', 'Pounds_CY'])
    
    results = []
    
    for idx, row in cust_item_volumes.iterrows():
        customer = row['customer']
        item = row['item']
        total_volume = row['total_volume']
        
        cust_item_data = df[
            (df[config.data_config.customer_id_column] == customer) &
            (df[item_col] == item)
        ]
        
        if len(cust_item_data) < 10:  # Need decent sample size
            continue
        
        # Calculate correlation between price and volume
        price_vol_corr = cust_item_data[['Net_Sales_Per_LB', 'Pounds_CY']].corr().iloc[0, 1]
        avg_price = cust_item_data['Net_Sales_Per_LB'].mean()
        
        # Strong negative correlation = price-sensitive
        if price_vol_corr < -0.5:
            sensitivity = 'HIGH'
            action = '🎯 PROTECT - Highly price sensitive, avoid increases'
        elif price_vol_corr < -0.2:
            sensitivity = 'MEDIUM'
            action = '⚠️ MONITOR - Moderately sensitive'
        else:
            sensitivity = 'LOW'
            action = '💪 PRICING POWER - Could test small increase'
    ...
    if not result_df.empty:
        result_df = result_df.sort_values('total_volume_lbs', ascending=False)
        print(f"   ✅ Analyzed {len(result_df)} customer+item pairs")
        
        high_sensitivity = len(result_df[result_df['sensitivity'] == 'HIGH'])
        print(f"      • {high_sensitivity} HIGH sensitivity pairs (protect pricing)")
    else:
        print(f"   ⚠️ No customer+item elasticity calculated")
                ...
                if zones[i] != zones[i-1]:
                    ...
                    if to_zone < from_zone:  # Price went DOWN
                        if was_lapsed and came_back:
                            ...
                            # Customer was active, we dropped price, volume changed
                            # Need to check if it was a GOOD or BAD margin giveaway
                            
                            volume_lift_pct = ((post_avg_volume - pre_avg_volume) / pre_avg_volume) * 100 if pre_avg_volume > 0 else 0
                            
                            # Option 1: If you have margin data per customer
                            if config.data_config.has_margin_data and 'Margin_Per_LB_CY' in cust_data.columns:
                                # Get margin per lb at each zone
                                pre_margin_per_lb = cust_data[
                                    (cust_data['Zone_Suffix_Numeric'] == from_zone) & 
                                    (cust_data['Pounds_CY'] > 0)
                                ]['Margin_Per_LB_CY'].mean()
                                
                                post_margin_per_lb = cust_data[
                                    (cust_data['Zone_Suffix_Numeric'] == to_zone) & 
                                    (cust_data['Pounds_CY'] > 0)
                                ]['Margin_Per_LB_CY'].mean()
                                
                                # Calculate net margin impact
                                if pd.notna(pre_margin_per_lb) and pd.notna(post_margin_per_lb):
                                    old_profit = pre_avg_volume * pre_margin_per_lb
                                    new_profit = post_avg_volume * post_margin_per_lb
                                    net_margin_change = new_profit - old_profit
                                    
                                    if net_margin_change > 0:
                                        # GOOD GIVEAWAY: Net profit increased
                                        classification = 'YELLOW_FLAG_PROACTIVE_GAINER'
                                        weight = 0.85
                                        evidence = f"Active at Zone {int(from_zone)}, moved to Zone {int(to_zone)}, volume +{volume_lift_pct:.0f}%, net margin +${net_margin_change:.2f}"
                                    else:
                                        # BAD GIVEAWAY: Lost margin
                                        classification = 'RED_FLAG_PROACTIVE_FLAT'
                                        weight = 0.0
                                        evidence = f"Active at Zone {int(from_zone)}, moved to Zone {int(to_zone)}, volume +{volume_lift_pct:.0f}% but net margin LOST ${abs(net_margin_change):.2f} - MARGIN GIVEAWAY"
                                else:
                                    # Margin data missing, use volume threshold
                                    if volume_lift_pct >= 30:
                                        classification = 'YELLOW_FLAG_PROACTIVE_GAINER'
                                        weight = 0.80
                                        evidence = f"Active at Zone {int(from_zone)}, moved to Zone {int(to_zone)}, volume +{volume_lift_pct:.0f}% (margin data incomplete, likely profitable)"
                                    else:
                                        classification = 'RED_FLAG_PROACTIVE_FLAT'
                                        weight = 0.0
                                        evidence = f"Active at Zone {int(from_zone)}, moved to Zone {int(to_zone)}, volume +{volume_lift_pct:.0f}% - questionable margin impact"
                            
                            else:
                                # Option 2: No margin data - use volume lift threshold
                                if volume_lift_pct >= 30:
                                    # Big volume gain - likely worth it
                                    classification = 'YELLOW_FLAG_PROACTIVE_GAINER'
                                    weight = 0.80
                                    evidence = f"Active at Zone {int(from_zone)}, moved to Zone {int(to_zone)}, volume +{volume_lift_pct:.0f}% (likely profitable)"
                                elif volume_lift_pct >= 10:
                                    # Small gain - questionable
                                    classification = 'YELLOW_FLAG_PROACTIVE_GAINER'
                                    weight = 0.60
                                    evidence = f"Active at Zone {int(from_zone)}, moved to Zone {int(to_zone)}, volume +{volume_lift_pct:.0f}% (margin impact uncertain)"
                                else:
                                    # Minimal/negative gain - bad giveaway
                                    classification = 'RED_FLAG_PROACTIVE_FLAT'
                                    weight = 0.0
                                    evidence = f"Active at Zone {int(from_zone)}, moved to Zone {int(to_zone)}, volume stayed flat/declined - MARGIN GIVEAWAY"
                        else:
...
def calculate_dynamic_thresholds(current_df: pd.DataFrame, config: FoodserviceConfig) -> Dict[str, float]:
    """
    Calculate company-wide baseline lapse rate and derive thresholds.
    
    This replaces fixed 15%/30% thresholds with data-driven benchmarks.
    Updates automatically with every run.
    
    Returns:
        Dict with baseline_lapse_rate, high_concern_threshold, excellent_threshold
    """
    
    print("\n📊 Calculating dynamic lapse rate thresholds...")
    
    # Ensure we have customer status
    if 'Customer_Status' not in current_df.columns:
        print("   ⚠️  No Customer_Status column - using fallback thresholds")
        return {
            'baseline_lapse_rate': 0.20,  # Fallback: assume 20% baseline
            'high_concern_threshold': 0.30,
            'excellent_threshold': 0.16,
            'acceptable_range': (0.18, 0.22)
        }
    
    # Calculate overall baseline
    total_customers = current_df.groupby(config.data_config.customer_id_column)['Customer_Status'].first()
    lapsed_count = (total_customers == 'LAPSED_FROM_CATEGORY').sum()
    total_count = len(total_customers)
    
    baseline_lapse_rate = lapsed_count / total_count if total_count > 0 else 0.20
    
    # Define thresholds relative to baseline
    high_concern_threshold = baseline_lapse_rate * 1.5   # 50% worse than normal = concern
    excellent_threshold = baseline_lapse_rate * 0.8      # 20% better than normal = excellent
    acceptable_upper = baseline_lapse_rate * 1.1         # Within 10% of normal = acceptable
    acceptable_lower = baseline_lapse_rate * 0.9
    
    thresholds = {
        'baseline_lapse_rate': baseline_lapse_rate,
        'high_concern_threshold': high_concern_threshold,
        'excellent_threshold': excellent_threshold,
        'acceptable_range': (acceptable_lower, acceptable_upper)
    }
    
    print(f"   ✅ Company-wide baseline lapse rate: {baseline_lapse_rate:.1%}")
    print(f"   📈 High concern threshold (1.5x baseline): {high_concern_threshold:.1%}")
    print(f"   🌟 Excellent threshold (0.8x baseline): {excellent_threshold:.1%}")
    print(f"   ✔️  Acceptable range: {acceptable_lower:.1%} - {acceptable_upper:.1%}")
...
class FoodserviceZoneEngine:
    ...
    def generate_recommendations(self, current_df: pd.DataFrame, 
        ...
        thresholds = calculate_dynamic_thresholds(current_df, self.config)
        baseline_lapse_rate = thresholds['baseline_lapse_rate']
        high_concern_threshold = thresholds['high_concern_threshold']
        ...
        for combo in current_df['Company_Combo_Key'].unique():
            ...
            # Calculate how combo compares to baseline
            lapse_ratio = lapsed_pct / baseline_lapse_rate if baseline_lapse_rate > 0 else 1.0
            
            # Determine recommendation type based on dynamic thresholds
            if lapsed_pct >= high_concern_threshold and current_zone > optimal_zone:
                # SIGNIFICANTLY WORSE than company average - HIGH PRIORITY
                ...
                if has_reactive_evidence:
                    rec.update({
                        ...
                        'stakeholder_message': (
                            ...
                            f"Lapse rate {lapsed_pct:.1%} is {lapse_ratio:.1f}x worse than company average ({baseline_lapse_rate:.1%}). "
                            f"Strong evidence price is the issue."
                        ),
                    ...
                    rec.update({
                        'recommendation_type': 'HIGH_RECOVERY_POTENTIAL',
                        'stakeholder_message': (
                            f"🤔 TEST OPPORTUNITY: Lapse rate {lapsed_pct:.1%} is {lapse_ratio:.1f}x worse than company average ({baseline_lapse_rate:.1%}). "
                            f"{lapsed_customers} customers stopped buying. Testing Zone {int(optimal_zone)} to see if price is the issue."
                        ),
                ...
                # Reactive pattern detected by earlier analysis
                ...
                # High reactive recovery rate (20%+ customers came back at lower zone)
                rec.update({
                    'recommendation_type': 'REACTIVE_CORRECTION',
                    'stakeholder_message': (
                        f"⚠️ {reactive_recovery_count} customers ({reactive_rate:.0%}) lapsed at Zone {int(current_zone)}, "
                        f"then came back at Zone {int(optimal_zone)}. This proves Zone {int(current_zone)} was overpriced."
                    ),
            ...
            elif green_flag_rate_journey >= 0.30 and current_zone > optimal_zone:
                # 30%+ customers are green flags at optimal zone (proven to work)
                rec.update({
            ...
            elif lapsed_pct > baseline_lapse_rate and current_zone > optimal_zone:
                # ABOVE AVERAGE lapse rate (but not extreme) - MEDIUM PRIORITY
                rec.update({
                    'recommendation_type': 'ABOVE_AVERAGE_LAPSE',
                    'stakeholder_message': (
                        f"📊 Lapse rate {lapsed_pct:.1%} is {lapse_ratio:.1f}x company average ({baseline_lapse_rate:.1%}). "
                        f"Zone {int(optimal_zone)} may reduce churn."
                    ),
                    'expected_result': f"Reduce churn = +{int(total_volume * 0.12):,} lbs",
                    'timeline': '📅 4-6 weeks',
                    'risk_level': '✅ LOW',
                    'implementation_priority': 65
                })

            elif current_zone > optimal_zone:
                # Generic improvement (lapse rate normal or better, but optimal zone is lower)
                rec.update({
                    ...
                    'stakeholder_message': (
                        ...
                        f"Lapse rate {lapsed_pct:.1%} is within normal range ({baseline_lapse_rate:.1%})."
                    ),
                ...
                # Current zone equals optimal, or optimal is higher (never recommend up)
                rec.update({
                    'recommendation_type': 'MONITOR',
                    'stakeholder_message': f"Zone {int(current_zone)} performing adequately (lapse rate: {lapsed_pct:.1%} vs baseline: {baseline_lapse_rate:.1%})",
            ...
            # Add baseline comparison for transparency
            rec['lapse_vs_baseline'] = f"{lapse_ratio:.2f}x"
            rec['company_baseline_lapse'] = f"{baseline_lapse_rate:.1%}"
            ...
            # ✅ ADD TO FLAGS:
            # Check for Zone X→0 moves (FLAG but don't recommend)
            if optimal_zone == 0:
                print(f"   🚩 FLAGGED: Zone {int(current_zone)}→0 move detected (will not recommend)")
                ...
                zone_1_to_0_flags.append({
                    ...
                    'current_zone': int(current_zone),  # ← Changed from hardcoded 1
                    'suggested_zone': 0,
                    'total_customers': combo_data[self.config.data_config.customer_id_column].nunique(),
                    'total_volume': combo_data['Pounds_CY'].sum(),
                    'reason': f'Historical data suggests Zone 0, but policy prevents {int(current_zone)}→0 moves'  # ← Updated
                })
            ...
            #Track BOTH customer counts AND volume changes INCLUDING LOST CUSTOMERS
            customers_growing = 0
            customers_declining = 0
            customers_active = 0
            customers_left = 0  # ← NEW: Track customers who LEFT
            total_growth_volume = 0  
            total_decline_volume = 0
            lost_customer_volume = 0  # ← NEW: Volume from customers who LEFT
            ...
            for customer_id in combo_data[self.config.data_config.customer_id_column].unique():
                ...
                if pounds_cy > 0:  # Still buying this year
                    customers_active += 1
                    volume_change = pounds_cy - pounds_py
                    
                    if volume_change > 0:
                        customers_growing += 1
                        total_growth_volume += volume_change
                    elif volume_change < 0:
                        customers_declining += 1
                        total_decline_volume += abs(volume_change)
                
                elif pounds_py > 0:  # ✅ CRITICAL FIX: Customer LEFT (bought last year, NOT this year)
                    customers_left += 1
                    lost_customer_volume += pounds_py  # All their PY volume is LOST

            # ✅ Total distinct customers (active + left)
            total_distinct_customers = customers_active + customers_left

            if total_distinct_customers > 0:
                # Customer-based rates
                growth_rate = customers_growing / customers_active if customers_active > 0 else 0
                decline_rate = customers_declining / customers_active if customers_active > 0 else 0
                churn_rate = customers_left / total_distinct_customers  # ✅ NEW: % who LEFT
                
                # ✅ FIXED: Volume-based analysis INCLUDING LOST CUSTOMERS
                net_volume_change = total_growth_volume - total_decline_volume - lost_customer_volume
                net_change_pct = net_volume_change / total_volume if total_volume > 0 else 0

                print(f"   📊 Customer Analysis: {customers_growing} growing, {customers_declining} declining, {customers_left} LEFT (churn: {churn_rate:.1%})")
                print(f"   📈 Active Customer Rates: {growth_rate:.1%} growing | {decline_rate:.1%} declining")
                print(f"   💰 Volume Analysis: +{total_growth_volume:,.0f} lbs growing, -{total_decline_volume:,.0f} lbs declining, -{lost_customer_volume:,.0f} lbs LOST from churned customers")
                print(f"   💰 Net Volume Change (with churn): {net_volume_change:+,.0f} lbs ({net_change_pct:+.1%})")

                # ✅ CRITICAL: Check ALL warning signs before skipping
                if churn_rate > 0.10:  # 10%+ churn = problem
                    print(f"   🚨 CUSTOMER CHURN: {churn_rate:.1%} of customers LEFT - WILL RECOMMEND")
                    # Don't continue - fall through to recommendations
                elif decline_rate > 0.30:  # 30%+ of active customers declining = problem
                    print(f"   🚨 HIGH DECLINE RATE: {decline_rate:.1%} of active customers declining - WILL RECOMMEND")
                    # Don't continue - fall through to recommendations
                elif net_change_pct > 0.10 and decline_rate < 0.30:
                    print(f"   🎉 KEEPING ZONE: Net volume growing by {net_change_pct:+.1%} with healthy decline rate")
                    continue
                elif net_change_pct > -0.05 and growth_rate > 0.50 and churn_rate < 0.05 and decline_rate < 0.30:
                    print(f"   🤔 STABLE ZONE: Nearly flat but low churn and low decline")
                    continue
                elif lapsed_pct < baseline_lapse_rate and net_change_pct >= 0 and churn_rate < 0.05 and decline_rate < 0.30:
                    print(f"   ✅ KEEPING ZONE: Lapse {lapsed_pct:.1%} below baseline ({baseline_lapse_rate:.1%}) + stable + low churn + low decline")
                    continue
            ...
            # ✅ FIX: Check for high churn BEFORE skipping
            if current_zone == optimal_zone:
                if churn_rate > 0.15:
                    new_zone = max(0, current_zone - 1)
                    
                    # ✅ Check if this would be a 1→0 move
                    if current_zone == 1 and new_zone == 0:
                        print(f"   🚩 FLAGGED: Zone 1→0 move needed (churn: {churn_rate:.1%}) - requires fractional zones")
                        zone_1_to_0_flags.append({
                            'company_combo': combo,
                            'company_name': combo_data[self.config.data_config.company_column].iloc[0],
                            'attribute_group': combo_data.get(self.config.data_config.attribute_group_column, pd.Series(['N/A'])).iloc[0],
                            ...
                            'reason': f'High churn ({churn_rate:.1%}) at Zone 1 suggests Zone 0, but policy requires fractional zones'
                        })
                        continue  # Skip to next combo - don't recommend
                    else:
                        print(f"   🚨 HIGH CHURN at 'optimal' zone ({churn_rate:.1%}) - recommending Zone {new_zone}")
                        optimal_zone = new_zone
                        # Don't continue - fall through to create recommendation
                    # Don't continue - let it fall through to create recommendation
                elif combo in self.reactive_flags:
                    print(f"   ⚠️ REACTIVE PRICING detected - recommending Zone {max(0, current_zone - 1)}")
                    optimal_zone = max(0, current_zone - 1)
                    # Don't continue - let it fall through
                else:
                    print(f"   ⏭️  SKIPPED: Already at optimal zone (low churn, no reactive flags)")
                    continue
...
def create_volume_decline_dashboard(df: pd.DataFrame, 
                                    config: FoodserviceConfig,
                                    top_n: int = 10) -> pd.DataFrame:  # Changed default to 10
    """Show biggest volume losses by combo + zone - ONLY actionable opportunities (Zone 2+)."""
    ...
    combo_zone = df.groupby(
        ...
        'Pounds_CY': 'sum',
        'Pounds_PY': 'sum',
        config.data_config.customer_id_column: 'nunique'
    }).reset_index()
    ...
    # Drop the third customer column (total)
    ...
    # ============================================
    # FILTER TO ACTIONABLE DECLINES ONLY
    # ============================================
    declines = combo_zone[
        (combo_zone['Volume_Change_Lbs'] < -100) &          # Lost 100+ lbs (material)
        (combo_zone['Pounds_CY'] >= 0) &                    # No negative volumes (data errors)
        (combo_zone['Pounds_PY'] > 0) &                     # Had volume last year (baseline)
        (combo_zone['Zone_Suffix_Numeric'] >= 2)            # Zone 2+ only (can actually move down)
    ].copy()
    
    # Sort by WORST declines (most negative volume loss)
    ...
    print(f"   ✅ Found {len(declines)} actionable combo-zone pairs with material volume declines")
...
def generate_high_value_leads(recommendations: List[Dict],
    ...
    for rec in high_priority_recs:
        ...
        combo_data['Net_Sales_PY'] = pd.to_numeric(combo_data.get('Net_Sales_PY', 0), errors='coerce').fillna(0)
        ...
        agg_dict = {
            'Net_Sales_CY': 'sum',
            'Net_Sales_PY': 'sum',  # ← ADD THIS LINE
            'Pounds_CY': 'sum',
            'Pounds_PY': 'sum',
            'Customer Name': 'first'
        }
        ...
        customer_summary['Volume_Decline_Lbs'] = customer_summary['Pounds_PY'] - customer_summary['Pounds_CY']
        customer_summary['Volume_Decline_Pct'] = (
            (customer_summary['Volume_Decline_Lbs'] / customer_summary['Pounds_PY'].replace(0, 1)) * 100
        )
        
        # Revenue for context only
        customer_summary['Max_Net_Sales'] = customer_summary[['Net_Sales_CY', 'Net_Sales_PY']].max(axis=1)
        customer_summary['Estimated_Annual_Revenue'] = customer_summary['Max_Net_Sales'] * 6.5
        
        # ============================================
        # FILTER BY VOLUME (not inflated revenue!)
        # ============================================
        min_volume_threshold = 500  # Must have bought 500+ lbs/year to be worth pursuing
        declining_customers = customer_summary[
            (
                (customer_summary['Customer_Status'] == 'LAPSED_FROM_CATEGORY') |
                (customer_summary['Volume_Decline_Lbs'] > 0)
            ) &
            (customer_summary['Pounds_PY'] >= min_volume_threshold)
        ].copy()
        
        print(f"      📊 Combo {combo}: {len(declining_customers)} high-volume declining customers")
        
        if declining_customers.empty:
            continue
        
        # Sort by volume decline
        declining_customers = declining_customers.sort_values('Volume_Decline_Lbs', ascending=False)
        
        # ============================================
        # CREATE LEADS
        # ============================================
        
        for _, cust in declining_customers.iterrows():  # ← Changed from "lapsed" to "declining_customers"
            
            if cust['Customer_Status'] == 'LAPSED_FROM_CATEGORY':
                lead_type = 'LAPSED_RECOVERY'
                priority = 'HIGH'
                reason = f"Stopped buying. Lost {cust['Pounds_PY']:,.0f} lbs/year."
                talking_point = f"You used to buy {cust['Pounds_PY']:,.0f} lbs but stopped. New Zone {recommended_zone} pricing available."
            
            elif cust['Volume_Decline_Pct'] >= 50:
                lead_type = 'STEEP_DECLINE'
                priority = 'HIGH'
                reason = f"Volume down {cust['Volume_Decline_Pct']:.0f}% ({cust['Volume_Decline_Lbs']:,.0f} lbs lost)."
                talking_point = f"Volume dropped {cust['Volume_Decline_Pct']:.0f}%. New Zone {recommended_zone} pricing to help."
            
            else:
                lead_type = 'MODERATE_DECLINE'
                priority = 'MEDIUM'
                reason = f"Volume down {cust['Volume_Decline_Pct']:.0f}% ({cust['Volume_Decline_Lbs']:,.0f} lbs)."
                talking_point = f"Volume down {cust['Volume_Decline_Pct']:.0f}%. Improved Zone {recommended_zone} pricing."
            ...
            leads_data.append({
                ...
                'Pounds_CY': f"{cust['Pounds_CY']:,.0f}",
                'Pounds_PY': f"{cust['Pounds_PY']:,.0f}",
                'Volume_Decline_Lbs': f"{cust['Volume_Decline_Lbs']:,.0f}",
                'Volume_Decline_Pct': f"{cust['Volume_Decline_Pct']:.0f}%",
                'Est_Annual_Revenue': f"${cust['Estimated_Annual_Revenue']:,.0f}",
                ...
                'Reason': reason,
                'Sales_Talking_Point': talking_point,
                'Expected_Action': 'Call, acknowledge volume decline, present new pricing'
            })
        
        total_customers_identified += len(declining_customers)
...
def create_how_to_use_tab(writer, recommendations_count, high_priority_count, volume_at_stake):
    ...
    tabs = [
        ...
        ("Tab 7: Zone Stickiness", "Which zones keep customers coming back week after week"),
        ("Tab 8: Volume Declines", "Biggest losses in pounds - where business is bleeding"),
        ("Tab 9: Financial Impact", "Dollar projections - revenue and margin impact estimates"),
        ("Tab 10: Item Elasticity", "Top 20 items by volume - which specific products are price-sensitive"),
        ("Tab 11: Customer+Item Sensitivity", "Top 20 customer+item pairs - who to protect from price increases"),
        ("Tab 12: Learning Tracker", "(Empty first time) Tracks how accurate our predictions were")
    ]
    ...
    steps = [
        ...
        ("STEP 3: Review Tab 9 (Financial Impact)", "See how much revenue and margin you'll gain from these changes."),
...
def write_stakeholder_excel(recommendations: List[Dict], 
    ...
    with ExcelWriter(output_path, engine='openpyxl') as writer:
        ...
        # Build display_cols from what actually exists
        display_cols = [col for col in key_cols if col in all_recs.columns]
        
        # Check for territory columns and add if they exist
        territory_cols = []
        if 'territory_flag' in all_recs.columns:
            territory_cols.append('territory_flag')
        if 'territory_rank' in all_recs.columns:
            territory_cols.append('territory_rank')
        if 'rep_guidance' in all_recs.columns:
            territory_cols.append('rep_guidance')
        
        # Combine all columns that exist
        all_display_cols = display_cols + territory_cols
        
        if all_display_cols:
            all_recs_display = all_recs[all_display_cols].copy()
            
            # Build column names dynamically based on what we have
            new_col_names = [
            ...
            # Add territory column names only if they exist
            if 'territory_flag' in territory_cols:
                new_col_names.append('Territory Flag')
            if 'territory_rank' in territory_cols:
                new_col_names.append('Territory Rank')
            if 'rep_guidance' in territory_cols:
                new_col_names.append('Rep Guidance')
            
            all_recs_display.columns = new_col_names
            ...
            note_col = len(new_col_names) + 1  # Next column after last data column
        ...
        # ==========================================
        # TAB 10: ITEM-LEVEL ELASTICITY
        # ==========================================
        print("   📄 Tab 10: Price Elasticity (Item Level - Top 20)")
        item_elasticity_df = calculate_item_level_elasticity(
            historical_df if historical_df is not None else current_df,
            optimization_engine.config,  # ✅ Change config to optimization_engine.config
            top_n=20
        )
        
        if not item_elasticity_df.empty:
            # Format for display
            display = item_elasticity_df[[
                'item', 'combo', 'total_volume_lbs', 'sensitivity', 'elasticity',
                'price_before', 'price_after', 'price_change_pct',
                'volume_before', 'volume_after', 'volume_change_pct'
            ]].copy()
            
            display.columns = [
                'Item (SUPC)', 'Combo', 'Total Volume (lbs)', 'Sensitivity', 'Elasticity',
                'Price Before', 'Price After', 'Price Change %',
                'Volume Before', 'Volume After', 'Volume Change %'
            ]
            
            # Format numbers
            display['Total Volume (lbs)'] = display['Total Volume (lbs)'].apply(lambda x: f"{x:,.0f}")
            display['Elasticity'] = display['Elasticity'].apply(lambda x: f"{x:.2f}")
            display['Price Before'] = display['Price Before'].apply(lambda x: f"${x:.2f}")
            display['Price After'] = display['Price After'].apply(lambda x: f"${x:.2f}")
            display['Price Change %'] = display['Price Change %'].apply(lambda x: f"{x:+.1f}%")
            display['Volume Before'] = display['Volume Before'].apply(lambda x: f"{x:,.0f}")
            display['Volume After'] = display['Volume After'].apply(lambda x: f"{x:,.0f}")
            display['Volume Change %'] = display['Volume Change %'].apply(lambda x: f"{x:+.1f}%")
            
            display.to_excel(writer, sheet_name='10_ELASTICITY_ITEMS', index=False)
        
        # ==========================================
        # TAB 11: CUSTOMER+ITEM SENSITIVITY
        # ==========================================
        print("   📄 Tab 11: Customer+Item Price Sensitivity (Top 20)")
        cust_item_df = calculate_customer_item_elasticity(
            historical_df if historical_df is not None else current_df,
            optimization_engine.config,  # ✅ Get config from engine
            top_n=20
        )
        
        if not cust_item_df.empty:
            display = cust_item_df[[
                'customer', 'item', 'total_volume_lbs', 'sensitivity',
                'avg_price_per_lb', 'price_vol_correlation', 'recommended_action'
            ]].copy()
            
            display.columns = [
                'Customer Number', 'Item (SUPC)', 'Volume (lbs)', 'Sensitivity',
                'Avg Price/LB', 'Price-Volume Correlation', 'Recommended Action'
            ]
            
            # Format numbers
            display['Volume (lbs)'] = display['Volume (lbs)'].apply(lambda x: f"{x:,.0f}")
            display['Avg Price/LB'] = display['Avg Price/LB'].apply(lambda x: f"${x:.2f}")
            display['Price-Volume Correlation'] = display['Price-Volume Correlation'].apply(lambda x: f"{x:.2f}")
            
            display.to_excel(writer, sheet_name='11_CUST_ITEM_SENSITIVITY', index=False)

        # ==========================================
        # TAB 12: Learning Tracker (if available)
        # ==========================================
        if learning_engine:
            print("   📄 Tab 12: Learning Tracker")
...
def run_foodservice_zone_optimization(
    ...
    # Check if filtering for single OpCo
    unique_opcos = set(rec['company_name'] for rec in recommendations)

    if len(unique_opcos) == 1:
        print(f"\n✅ Single OpCo analysis ({list(unique_opcos)[0]}) - skipping territory-relative scoring")
        print(f"   💡 All {len(recommendations)} opportunities will be included")
    else:
        print(f"\n🎯 Applying territory-relative scoring across {len(unique_opcos)} OpCos...")
    ...
    elasticity_df = calculate_price_elasticity(historical_df, config)
...
if __name__ == "__main__":
    ...
    HISTORICAL_DATA = [
        r"C:\Users\kmor6669\OneDrive - Sysco Corporation\Desktop\Pricing\202516_202617.csv",
        r"C:\Users\kmor6669\OneDrive - Sysco Corporation\Desktop\Pricing\202416_202515.csv"
    ]
