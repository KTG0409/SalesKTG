...
# ================= USER CONFIG =================
BASE_PATH = r"C:\Users\kmor6669\Sysco Corporation\SBC APP - Pictures\Seafood\Shrimp_Prawns"

DEFAULTS = {
    "source": None,  # ← Change from specific file to None
    "alignment": r"C:\Users\kmor6669\Sysco Corporation\SBC APP - Pictures\Seafood\Shrimp_Prawns\Alignment550.csv",
    "outdir": r"C:\Users\kmor6669\Sysco Corporation\SBC APP - Pictures\Seafood\Shrimp_Prawns\Outputs",
    "watch_dir": r"C:\Users\kmor6669\Sysco Corporation\SBC APP - Pictures\Seafood\Shrimp_Prawns\Inputs",  
...
def build_summary(status, current_week):
    ...
    kpi = pd.DataFrame([{
        ...
        "Customers CY": status["Distinct_Customers_CY"].sum(),  # NEW
        "Customers PY": status["Distinct_Customers_PY"].sum(),  # NEW
        "Customer Retention %": pct(status["Distinct_Customers_CY"].sum(), status["Distinct_Customers_PY"].sum()),  # NEW
    }])
...
def _write_weekly_chart_only_tab(xw, sheet_name, weekly_df, forecast_method_desc, filtered_company=None):
    ...
    # DESCRIPTIVE TITLE
    if filtered_company:
        bar.title = (f"Weekly Performance - {filtered_company}\n"
                    f"Bars = Change vs Last Year  |  Lines = Current Year Volume & Forecast  |  Green Line = Growth %")
    else:
...
def create_guide_tab(xw, status_df, weeks_covered, filtered_company_name=None, min_week=1, max_week=52):
    ...
    # === REPORT HEADER ===
    row = 1
    ws.cell(row, 1, value="📊 HOW TO USE THIS CATEGORY PERFORMANCE REPORT").font = title_font
   ...
   # === SECTION 3: TAB-BY-TAB GUIDE ===
    ws.cell(row, 1, value="📑 HOW TO USE EACH TAB").font = header_font
    ws.cell(row, 1).fill = header_fill
    ws.merge_cells(f"A{row}:G{row}")
    row += 1
    ...
    # Then your existing Growth Formula section continues...
    # === SECTION 5: THE GROWTH FORMULA (CORRECTED) ===
    ws.cell(row, 1, value="📈 THE CATEGORY GROWTH FORMULA (In Priority Order)").font = header_font
    ws.cell(row, 1).fill = header_fill
    ws.merge_cells(f"A{row}:G{row}")
    row += 1
    ...
    ws.cell(row, 1, value="❓ FREQUENTLY ASKED QUESTIONS").font = header_font
    ws.cell(row, 1).fill = header_fill
    ws.merge_cells(f"A{row}:G{row}")
    row += 1
    ...
    ws.cell(row, 1, value="📞 NEED HELP?").font = header_font
    ws.cell(row, 1).fill = header_fill
    ws.merge_cells(f"A{row}:G{row}")
    row += 1
    ...
    for row_num in range(1, row + 1):
        if ws.cell(row_num, 1).font.size and ws.cell(row_num, 1).font.size >= 14:
...
def build_dsm_opportunity_scorecard(status_df, raw_df, source_path, active_weeks=8, min_ytd=260):
    ...
    # ... rest of function unchanged (conversion uses status_df as before)
    ...
    for dsm in dsm_summary["Customer DSM"].head(20):
        ...
        winback_targets_list.append(dsm_winbacks[[
    ...
    for dsm in dsm_summary["Customer DSM"].head(20):
        ...
        conversion_targets_list.append(dsm_conversions[[
            "Customer DSM", "Customer Name", "Company Customer Number", "Customer Territory",
...
def main():
    ...
    write_excel(
        ...
        summary_kpi, lag_sites, cust_losses,
        status, leads, vendor_index,
        company_yoy=company_yoy,              # WAS None
        all_weekly=all_weekly,                # WAS None
        trs_weekly=trs_weekly,                # WAS None
        cmu_weekly=cmu_weekly,                # WAS None
        lcc_weekly=lcc_weekly,                # WAS None
        forecast_method=forecast_method,      # KEEP AS IS
        status_raw=raw,                       # WAS None
        alignment_df=alignment_df,            # WAS None
        filtered_company_name=filtered_company_name,  # WAS None
        min_ytd=min_ytd,                      # KEEP AS IS
