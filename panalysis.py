...
def calculate_yoy_customer_metrics(df: pd.DataFrame, 
    ...
    # DEBUG: Verify distinct count
    print(f"   🔍 DEBUG YoY:")
    print(f"      Recent data rows: {len(recent_data):,}")
    print(f"      Rows with Pounds_CY > 0: {len(recent_data[recent_data['Pounds_CY'] > 0]):,}")
    print(f"      DISTINCT customers CY: {len(cy_customers):,}")
    print(f"      Rows with Pounds_PY > 0: {len(recent_data[recent_data['Pounds_PY'] > 0]):,}")
    print(f"      DISTINCT customers PY: {len(py_customers):,}")
...
class FoodserviceZoneEngine:
    ...
    def generate_recommendations(self, current_df: pd.DataFrame, 
        ...
        for combo in current_df['Company_Combo_Key'].unique():
            ...
            # ⚠️ FLAG ZONE 0 (mark for special handling later)
            zone_0_flag = False
            if optimal_zone == 0:
                print(f"   🚩 FLAGGED: Zone 0 optimal → Need fractional zones")
                zone_0_flag = True  # Don't continue - let it build the rec!
...
# Skip if no change needed
            ...
            # ⚠️ Override for Zone 0 recommendations
            if zone_0_flag:
                rec['requires_fractional'] = True
                rec['recommendation_type'] = 'NEEDS_FRACTIONAL_ZONES'
                rec['stakeholder_message'] = (
                    f"🚩 Zone 1 too expensive, Zone 0 gives away profit. "
                    f"Need fractional zones (0.5, 0.75) for better pricing granularity."
                )
                rec['implementation_priority'] = 85
...
def create_executive_summary(recommendations: List[Dict], 
    ...
    # Add YoY metrics
    if yoy_metrics:
        lookback_weeks = yoy_metrics.get('lookback_weeks', 8)
        summary.update({
            'yoy_comparison_header': f'📊 {lookback_weeks}-WEEK BUYING PATTERN COMPARISON',
            'yoy_comparison_period': f"Weeks {yoy_metrics.get('week_range', 'N/A')}",
            'active_buyers_this_period': f"{yoy_metrics['distinct_customers_cy']:,}",
            'active_buyers_same_period_last_year': f"{yoy_metrics['distinct_customers_py']:,}",
            'repeat_buyers': f"{yoy_metrics['retained_customers']:,} ({yoy_metrics['retention_rate']:.1f}%)",
            'first_time_buyers_this_period': f"{yoy_metrics['new_customers']:,}",
            'lapsed_buyers_this_period': f"{yoy_metrics['lost_customers']:,}",
            'yoy_note': f"⚠️ Measures buying frequency in {lookback_weeks}-week window. Lapsed buyers may have purchased in other periods."
        })
